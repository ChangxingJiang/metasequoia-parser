...
class Item1Set:
    ...
    other_item_set: Set[Item1] = dataclasses.field(kw_only=True)  # 项目集闭包中除核心项目外的其他等价项目
    successor_hash: Dict[int, "Item1Set"] = dataclasses.field(kw_only=True, default_factory=lambda: {})  # 后继项目的关联关系

    @staticmethod
    def create(core_list: Tuple[Item1], other_item_set: Set[Item1]) -> "Item1Set":
        """项目集闭包对象的构造方法"""
        return Item1Set(
            core_tuple=core_list,
            other_item_set=other_item_set
        )

    @property
    def all_item_list(self) -> List[Item1]:
        """返回项目集中的所有项目的列表，包括核心项目和其他等价项目；在输出的列表中，第 0 个项目为核心项目"""
        return list(self.core_tuple) + list(self.other_item_set)
